...
[CustomEditor(typeof(MeshCombinerForGroups))]
public class MeshCombinerForGroupsEditor : Editor
{
    public override void OnInspectorGUI()
    {
        MeshCombinerForGroups meshCombiner = (MeshCombinerForGroups)target;
        Mesh mesh = meshCombiner.GetComponent<MeshFilter>().sharedMesh;

        #region Script:
        GUI.enabled = false;
        EditorGUILayout.ObjectField("Script", MonoScript.FromMonoBehaviour((MeshCombinerForGroups)target), typeof(MeshCombinerForGroups), false);
        GUI.enabled = true;
        #endregion Script.
        ...
        EditorGUI.BeginChangeCheck();
        EditorGUILayout.PropertyField(group, true);
        if(EditorGUI.EndChangeCheck())
        {
            serializedObject.ApplyModifiedProperties();
        }
