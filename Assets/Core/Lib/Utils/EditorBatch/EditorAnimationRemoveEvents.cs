...
namespace Core.Lib
{
    public class EditorAnimationRemoveEvents : AbstractEditorBatchExecute
    {
        ...
        protected override IEnumerator ExecuteProtected(Transform[] suitable, Action next, Action cancel)
        {
            ...
            foreach (var ev in _events)
            {
                ...
                if (selfEvents.All(i => i.objectReferenceParameter != ev.obj))
                    continue;

                selfEvents = selfEvents.Where(i => i.objectReferenceParameter != ev.obj).ToList();
